...
function buildMedals(bestScores: Map<number, number>): MissionMedal[] {
  return MISSIONS.map(m => {
    ...
    return { missionId: m.id, missionName: m.name, missionIcon: m.icon,
             medal: m.hasEval ? medalFromPercentage(pct) : 'none', percentage: pct, hasEval: m.hasEval };
  });
}
...
export class TrophiesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (!this.isAdmin) {
      ...
      // ── Suscripción reactiva: al ganar un trofeo nuevo, refrescar vista ──
      this.gamSub = this.gamification.trophies$.subscribe(trophies => {
        this.recentTrophies = trophies;
        // Refrescar medallas si hay nuevos resultados en la sesión
  ...
  private loadDocenteView(): void {
    ...
    this.evalService.getResultsByUser(user.userId).subscribe({
      ...
      error: () => { this.myMedals = buildMedals(new Map()); this.loading = false; }
    });
  }
    ...
    this.evalService.getAllResults().subscribe({
      ...
      error: () => {
        this.docenteRows = registered.map(d => ({
          userId: d.userId, displayName: d.displayName, medals: buildMedals(new Map()), totalEarned: 0
        }));
        this.loading = false;
      }
    });
