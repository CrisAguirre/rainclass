...
export class CollectiblesComponent implements OnInit {
  ...
  private buildCollectibles(completedIds: number[], scores: { labId: number; percentage: number }[]): Collectible[] {
    return [
      { id: 'c1', name: 'Cubo Mágico', icon: '🧊', description: 'Completa la misión Merge Cube', rarity: 'comun', earned: completedIds.includes(1) },
      { id: 'c2', name: 'Pincel Digital', icon: '🖌️', description: 'Completa la misión QuiverVision', rarity: 'comun', earned: completedIds.includes(2) },
      { id: 'c3', name: 'Mapa del Tesoro', icon: '🗺️', description: 'Completa la misión Actionbound', rarity: 'raro', earned: completedIds.includes(3) },
      { id: 'c4', name: 'Visor VR', icon: '🥽', description: 'Completa la misión Metaverso Meta', rarity: 'raro', earned: completedIds.includes(4) },
      { id: 'c5', name: 'Generador RA', icon: '🔬', description: 'Completa la misión RA Propia', rarity: 'epico', earned: completedIds.includes(5) },
      { id: 'c6', name: 'Ancla Geoespacial', icon: '📍', description: 'Completa la misión Geoposición', rarity: 'epico', earned: completedIds.includes(6) },
    ...
    this.evalService.getAllResults().subscribe({
      next: (results) => {
        ...
        this.docenteList = Array.from(userMap.entries()).map(([userId, data]) => {
          const cols = this.buildCollectibles([], data.scores);
